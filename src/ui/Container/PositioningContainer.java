...
public class PositioningContainer extends UIContainer {
    ...
    // does not change the position of this container itself but specifies how the children components should be offset
    // can use anchor because inverting it and add its own offset to its children from the anchor will produce the result of
    // positioning the object relative to that point within the container
    // i.e. TOP_CENTER will draw all elements relative to the top center of this container
    protected Anchor drawPosition;

    // if true, every child added to container will have thier anchor set to the same anchor as the drawPosition
    ...
    // inverting Anchor offset methods will move objects away from origin which in this case is the top left of the screen
    @Override
